...
public class LeitnerSystem extends StudyMethod {
    ...
    // Moved method from StudyCardsController
    public String getRandomCard(List<Box> otherBoxes) {
        ...
        return buildCardResponse(randomCard, card);
    }
    ...
    public String getRandomCardFromBox() {
        // Seleciona aleatoriamente uma caixa
        int randomBoxIndex = (int) (Math.random() * boxes.size());
        Box selectedBox = boxes.get(randomBoxIndex);

        // Obtém uma carta aleatória da caixa selecionada
        Integer randomCardId = selectedBox.getRandomCard();

        if (randomCardId == null) {
            return "No card found";
        }

        Card card = getCardFromManager(randomCardId);
        return buildCardResponse(randomCardId, card);
    }
    ...
    private String buildCardResponse(Integer randomCard, Card card) {
